...
Процедура ВывестиПодразделениеРекурсивно(ДокументРезультат, СтрокаПодразделения, Области, ПараметрыОтчета, Итоги = Неопределено)
	...
	Для каждого ДочерняяСтрока Из СтрокаПодразделения.Строки Цикл
		Если ЗначениеЗаполнено(ДочерняяСтрока.Сотрудник) Тогда
			ВывестиСотрудника();
			Продолжить;
		КонецЕсли;
		...
		ВывестиПодразделениеРекурсивно(ДокументРезультат, ДочерняяСтрока, Области, ПараметрыОтчета, Итоги);
	КонецЦикла;
