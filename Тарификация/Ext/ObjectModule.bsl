...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	ДополнитьОписаниеПараметровНачислениями(ПараметрыОтчета);
...
Процедура СформироватьТабличныйДокумент(ДокументРезультат, ДанныеОтчета, ПараметрыОтчета)
	...
	Для каждого СтрокаОрганизации Из ДанныеОтчета.Строки Цикл
		ДокументРезультат.Вывести(Области.Шапка, 0);
...
Процедура ВывестиПодразделения(ДокументРезультат, СтрокиПодразделений, Области, ПараметрыОтчета, Итоги = Неопределено)
	...
	Для каждого СтрокаПодразделения Из СтрокиПодразделений Цикл
		ВывестиПодразделениеРекурсивно(ДокументРезультат, СтрокаПодразделения, Области, ПараметрыОтчета, 1, Итоги);
	КонецЦикла;

КонецПроцедуры

Процедура ВывестиПодразделениеРекурсивно(ДокументРезультат, СтрокаПодразделения, Области, ПараметрыОтчета, Уровень, Итоги = Неопределено)

	Области.Подразделение.Параметры.Заполнить(СтрокаПодразделения);
	ДокументРезультат.Вывести(Области.Подразделение, Уровень);
	
	ИтогиТекущегоПодразделения = СтруктураИтога();
	
	ТабдокДочерние = Новый ТабличныйДокумент;
	ТабдокДочерние.НачатьАвтогруппировкуСтрок();
	Для каждого ДочерняяСтрока Из СтрокаПодразделения.Строки Цикл
		Если ДочерняяСтрока.Сотрудник <> Неопределено Тогда
			ВывестиСотрудника(ДокументРезультат, ДочерняяСтрока, Области, ПараметрыОтчета, Уровень + 1, ИтогиТекущегоПодразделения);
			Продолжить;
		КонецЕсли;
		...
		Если ДочерняяСтрока.Подразделение = СтрокаПодразделения.Подразделение Тогда
			Для каждого СтрокаСотрудника Из ДочерняяСтрока.Строки Цикл
				ВывестиСотрудника(ДокументРезультат, СтрокаСотрудника, Области, ПараметрыОтчета, Уровень + 1, ИтогиТекущегоПодразделения);
			КонецЦикла;
			Продолжить;	
		КонецЕсли;
		
		ВывестиПодразделениеРекурсивно(ТабдокДочерние, ДочерняяСтрока, Области, ПараметрыОтчета, Уровень + 1, ИтогиТекущегоПодразделения);
	КонецЦикла;
	ТабдокДочерние.ЗакончитьАвтогруппировкуСтрок();
	
	ДокументРезультат.Вывести(ТабдокДочерние);
	
	Области.ИтогоПодразделение.Параметры.Заполнить(СтрокаПодразделения);
	Области.ИтогоПодразделение.Параметры.Заполнить(ИтогиТекущегоПодразделения);
	ДокументРезультат.Вывести(Области.ИтогоПодразделение, Уровень);
	
	Если Итоги <> Неопределено Тогда
		ДобавитьВИтог(Итоги, ИтогиТекущегоПодразделения);	
	КонецЕсли;

КонецПроцедуры

Процедура ВывестиСотрудника(ДокументРезультат, СтрокаСотрудника, Области, ПараметрыОтчета, Уровень, Итоги = Неопределено)
	...
	ДанныеСотрудника.Вставить("ДанныеСтажа", ПустойСтаж());
	ДанныеСотрудника.Вставить("ДанныеВыслуги", ПустойСтаж());
	Для каждого СтрокаНачисления Из СтрокаСотрудника.Строки Цикл
		...
		Если ИмяПараметра <> Неопределено Тогда
			...
			Если ИмяПараметра = "Стаж" Тогда
				ДанныеСотрудника.ДанныеСтажа = ДанныеСтажа(СтрокаНачисления.Строки, ДанныеСотрудника.ДанныеСтажа);
			КонецЕсли;
			
			Если ИмяПараметра = "Выслуга" Тогда
				ДанныеСотрудника.ДанныеВыслуги = ДанныеСтажа(СтрокаНачисления.Строки, ДанныеСотрудника.ДанныеВыслуги);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
	ДанныеСотрудника.Вставить("СтажСтаж", ПредставлениеСтажа(ДанныеСотрудника.ДанныеСтажа));
	ДанныеСотрудника.Вставить("ВыслугаСтаж", ПредставлениеСтажа(ДанныеСотрудника.ДанныеВыслуги));
	
	СведенияОЗваниях = Новый Массив;
	Если ЗначениеЗаполнено(ДанныеСотрудника.УченаяСтепень) Тогда
		СведенияОЗваниях.Добавить(Строка(ДанныеСотрудника.УченаяСтепень));
	КонецЕсли;
	Если ЗначениеЗаполнено(ДанныеСотрудника.Награда) Тогда
		СведенияОЗваниях.Добавить(Строка(ДанныеСотрудника.Награда));
	КонецЕсли;
	
	ДанныеСотрудника.Вставить("СведенияОЗваниях", СтрСоединить(СведенияОЗваниях, "; "));
	
	Области.Строка.Параметры.Заполнить(ДанныеСотрудника);
	ДокументРезультат.Вывести(Области.Строка, Уровень);
	
	Если Итоги <> Неопределено Тогда
		ДобавитьВИтог(Итоги, ДанныеСотрудника);	
	КонецЕсли;
	
КонецПроцедуры

Функция ДанныеСтажа(СтрокиПоказателей, ТекущиеДанные)
	
	Для каждого СтрокаПоказателя Из СтрокиПоказателей Цикл
		Стаж = ПустойСтаж();
		ЗаполнитьЗначенияСвойств(Стаж, СтрокаПоказателя);
		Если СтажБольше(Стаж, ТекущиеДанные) Тогда
			ТекущиеДанные = Стаж;		
		КонецЕсли;			
	КонецЦикла;	
...
Функция СтажБольше(Стаж1, Стаж2)
	
	Если Стаж1.Лет = Стаж2.Лет Тогда
		Если Стаж1.Месяцев = Стаж2.Месяцев Тогда
			Возврат Стаж1.Дней > Стаж2.Дней;		
		КонецЕсли;	
		Возврат Стаж1.Месяцев > Стаж2.Месяцев;
	КонецЕсли;
	
	Возврат Стаж1.Лет > Стаж2.Лет;
	
КонецФункции

Функция ПредставлениеСтажа(Стаж)

	Если Стаж.Лет = 0 и Стаж.Месяцев = 0 и Стаж.Дней = 0 Тогда
		Возврат "";	
	КонецЕсли;

	Возврат Формат(Стаж.Лет, "ЧН=0г; ЧФ=Чг") + Формат(Стаж.Месяцев, "ЧН=0м; ЧФ=Чм") + Формат(Стаж.Дней, "ЧН=0д; ЧФ=Чд")
...
Функция СтруктураИтога()

	Итог = Новый Структура();
	
	Итог.Вставить("ИтогоСПустыннойМестностью", 0);
	
	Возврат Итог;

КонецФункции // ()

Процедура ДобавитьВИтог(Итог, Данные)

	Для каждого Поле Из Итог Цикл
		Ключ = Поле.Ключ;
		Если Данные.Свойство(Ключ) И ТипЗнч(Данные[Ключ]) = Тип("Число") Тогда
			Итог[Ключ] = Итог[Ключ] + Данные[Ключ];
		КонецЕсли;
	КонецЦикла;
...
// Чтобы впоследствие, при разработке других отчетов на основе этого, не морочится с параметрами
#Область ПараметрыНачислений
Процедура ПриЗаполненииОписанияНачислений(ОписаниеНачислений)

	ДобавитьОписаниеНачисления(ОписаниеНачислений, "Должностной оклад", "Оклад");
	ДобавитьОписаниеНачисления(ОписаниеНачислений, "выплата за работу с вредными и (или) опасными условиями труда", "Вредность");
	ДобавитьОписаниеНачисления(ОписаниеНачислений, "надбавка за работу со сведениями, составляющими государственную тайну, их засекречиванием и рассекречиванием, а также за работу с шифрами", "Гостайна");
	ДобавитьОписаниеНачисления(ОписаниеНачислений, "за стаж непрерывной работы", "Стаж");
	ДобавитьОписаниеНачисления(ОписаниеНачислений, "за выслугу лет", "Выслуга");
	ДобавитьОписаниеНачисления(ОписаниеНачислений, "За квалификацию", "Квалификация");
	ДобавитьОписаниеНачисления(ОписаниеНачислений, "За наличие ученой степени", "УченаяСтепень");
	ДобавитьОписаниеНачисления(ОписаниеНачислений, "За наличие  почетного звания или ведомственного почетного звания (нагрудного знака)", "ПочетноеЗвание");
	ДобавитьОписаниеНачисления(ОписаниеНачислений, "Повышение заработной платы за работу на территориях, отнесенных к пустынной и безводной местности", "ПустыннаяМестность");

КонецПроцедуры

Процедура ДобавитьОписаниеНачисления(ОписаниеНачислений, Заголовок, Имя = Неопределено)

	НовоеНачисление = ОписаниеНачислений.Добавить();
	НовоеНачисление.Заголовок = Заголовок;
	Если Имя = Неопределено Тогда
		НовоеНачисление.Имя = ЗарплатаКадрыКлиентСервер.ИдентификаторМакетаПечатнойФормы(Заголовок);
	Иначе 
		НовоеНачисление.Имя = Имя;
	КонецЕсли;

КонецПроцедуры

Функция ОписаниеНачислений()

	ОписаниеНачислений = НовоеОписаниеНачислений();
	ПриЗаполненииОписанияНачислений(ОписаниеНачислений);
	
	Возврат ОписаниеНачислений;

КонецФункции // ОписаниеНачислений()

Функция НовоеОписаниеНачислений()

	ОписаниеНачислений = Новый ТаблицаЗначений;
	ОписаниеНачислений.Колонки.Добавить("Имя");
	ОписаниеНачислений.Колонки.Добавить("Заголовок");
	
	Возврат ОписаниеНачислений;

КонецФункции // НовоеОписаниеНачислений()

Процедура ДополнитьОписаниеПараметровНачислениями(ПараметрыОтчета)
	
	ОписаниеНачислений = ОписаниеНачислений();

	ПараметрыОтчета.Вставить("Начисления", Новый Структура);
	ПараметрыОтчета.Вставить("СоответствиеНачисления", Новый Соответствие);
	
	Для каждого Параметр Из ОписаниеНачислений Цикл
	
		ПараметрыОтчета.Начисления.Вставить(Параметр.Имя, Новый Массив);
	
	КонецЦикла;

КонецПроцедуры

Процедура ДобавитьПараметрыНачисленийВСКД()
	...
	НовыйПараметр.Имя = "ПараметрыНачислений";
	НовыйПараметр.Заголовок = "Параметры начислений";
	...
	Для каждого Параметр Из ОписаниеНачислений Цикл
		...
		НовыйПараметр.ТипЗначения = Новый ОписаниеТипов("ПланВидовРасчетаСсылка.Начисления");
...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	ДобавитьПараметрыНачисленийВСКД();
